...
def plot_classes(data, assignments):raise NotImplemented('disambiguate functions')
def plot_accuracy(): raise NotImplemented()
def plot_feats(replaceme): raise NotImplemented()
...
def plot_tsne3d(data):
    ...
    plt.show()
#######################################################################################################################
def plot_classes_EMGMM_assignments(data, assignments):
    ...
    for g in np.unique(assignments):
        ...
        ax.scatter(tsne_x[idx], tsne_y[idx], tsne_z[idx], c=cmap[g], label=g, s=0.5, marker='o', alpha=0.8)
    ax.set_xlabel('Dim. 1')
    ax.set_ylabel('Dim. 2')
    ax.set_zlabel('Dim. 3')
    ax.view_init(70, 135)
...
def plot_classes_bsoidvoc(data, assignments) -> None:
    return plot_classes_EMGMM_assignments(data, assignments)
def plot_classes_bsoidpy(data, assignments) -> None:
    return plot_classes_EMGMM_assignments(data, assignments)

# TODO: central difference b/w APP and UMAP is what is returned
def plot_classes_bsoidapp(data, assignments) -> Tuple:
    ...
    for g in np.unique(assignments):
        idx = np.where(np.array(assignments) == g)
        ax.scatter(umap_x[idx], umap_y[idx], umap_z[idx], c=cmap[g], label=g, s=0.5, marker='o', alpha=0.8)
    ax.set_xlabel('Dim. 1')
    ax.set_ylabel('Dim. 2')
    ax.set_zlabel('Dim. 3')
...
def plot_accuracy_bsoidapp(scores) -> Tuple:
    """This function is a traceback to the original bsoid_app function `plot_accuracy()`. In an
    effort to combine 5 different variantions of the same function, plot_accuracy_MLP was created.
    Before these garbage functions can be jettisoned, compatibility must be established"""
    # fig = plt.figure(facecolor='w', edgecolor='k')
...
def plot_accuracy_SVM(scores, save_fig_to_file=True, fig_file_prefix='clf_scores'):
    ...
    Currently it calls the OLD function just because the old function remains for backwards compatibility purposes
    and we are trying to DRY everything."""
    fig = plt.figure(facecolor='w', edgecolor='k')
    fig.suptitle(f"Performance on {HLDOUT * 100} % data")
    ax = fig.add_subplot(111)
    ax.boxplot(scores, notch=None)
...
#######################################################################################################################
def plot_durhist(lengths, grp, save_fig_to_file: bool = True, save_file_prefix='duration_hist_100msbins'):
    """
    TODO: purpose
    ...
    plt.show()
    fig.savefig(os.path.join(OUTPUT_PATH, f'{save_file_prefix}_{timestr}.svg'))
    return fig
#######################################################################################################################
def plot_feats_bsoidUMAPAPP(feats: list, labels: list) -> None:
    ...
    if result:
        ...
        plt.show()
def plot_feats_bsoidpy(feats: list, labels: list) -> None:
    """
    :param feats: list, features for multiple sessions
    :param labels: list, labels for multiple sessions
    """
