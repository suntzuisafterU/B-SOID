...
def repeatingNumbers(labels):  # TODO: rename function for clarity
    """
    TODO: low: purpose
...
def vid2frame(vidname, labels, fps, output_path=FRAME_DIR):
    ...
    width = vidobj.get(3)  # TODO: low: unused variable
    height = vidobj.get(4)  # TODO: low: unused variable
...
def import_vidfolders(folders: list, output_path: list):
    ...
    for i, fd in enumerate(folders):  # Loop through folders
        ...
        for j, vidname in enumerate(v):
            logging.info(f'Extracting frames from {vidname} and appending labels to these images...')
            vid2frame(vidname, output_path)  # TODO: HIGH missing param `FPS`
            logging.info('Done extracting images and writing labels, from MP4 file {}'.format(j + 1))
        vidnames.append(v)
        logging.info(f'Processed {len(v)} MP4 files from folder: {fd}')
    return
