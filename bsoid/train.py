...
def train_TSNE_NEW(list_of_arrays_data: pd.DataFrame, features, fps=config.VIDEO_FPS, dimensions: int = 3, **kwargs) -> Tuple[pd.DataFrame, pd.DataFrame, TSNE_bthsne, StandardScaler]:
    """
    ...
    for n, feature_n in enumerate(features):  # for n in range(len(features)):
        ...
        for k in range(round(fps / 10) - 1, len(feature_n[0]), round(fps / 10)):
            if k > round(fps / 10) - 1:
                features1 = np.concatenate((features1.reshape(features1.shape[0], features1.shape[1]), np.hstack((
                    np.mean((feature_n[0:4, range(k - round(fps / 10), k)]), axis=1),
                    np.sum((feature_n[4:7, range(k - round(fps / 10), k)]), axis=1),
                )).reshape(len(features[0]), 1)), axis=1)
            else:
                features1 = np.hstack((np.mean((feature_n[0:4, range(k - round(fps / 10), k)]), axis=1), np.sum((feature_n[4:7, range(k - round(fps / 10), k)]), axis=1))).reshape(len(features[0]), 1)
    ...
    scaler_obj.fit(features_10fps.T)  # TODO: HIGH: variable `f_10fps` referenced before assignment. Error in logic above? ########################## IMPORTANT ###############################
    ...
    trained_tsne = TSNE_bthsne(f_10fps_scaled.T, dimensions=dimensions, perplexity=np.sqrt(f_10fps_scaled.shape[1]), theta=0.5, rand_seed=config.RANDOM_STATE)
