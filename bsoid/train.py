...
########################################################################################################################
def train_TSNE(list_of_arrays_data: List[np.ndarray], features, fps=config.VIDEO_FPS, comp: int = config.COMPILE_CSVS_FOR_TRAINING):
    """
...
def extract_features_and_train_TSNE(list_of_arrays_data: List[np.ndarray], bodyparts=config.BODYPARTS_PY_LEGACY, fps=config.VIDEO_FPS, comp: int = config.COMPILE_CSVS_FOR_TRAINING) -> Tuple[Any, Any, Any, Any]:
    ...
    # 2/2 Train tsne
    features_10fps, f_10fps_scaled, trained_tsne, scaler = train_TSNE(list_of_arrays_data, features, fps=fps, comp=comp)
    ...
    if config.PLOT_GRAPHS:  # TODO: low: saving the plot requires the plot to be shown
        ...
        for title, normalize in titles_options:
            ...
            if config.SAVE_GRAPHS_TO_FILE:
                file_name = f'confusion_matrix_{title_names[j]}_{time_str}'  # my_file = f'confusion_matrix_{title_names[j]}' # disp.figure_.savefig(os.path.join(config.OUTPUT_PATH, f'{my_file}{time_str}.svg'))
...
def train_emgmm_with_learned_tsne_space(trained_tsne_array, comp=config.COMPILE_CSVS_FOR_TRAINING, emgmm_params=config.EMGMM_PARAMS) -> np.ndarray:
    ...
    if comp == 1:
        ...
        for assignment in tqdm(range(len(trained_tsne_array))):
            ...
            clf_gmm = mixture.GaussianMixture(**emgmm_params).fit(trained_tsne_array[assignment])
    ...
    else: raise ValueError(f'Comp value is invalid. Should be 0 or 1. Actually is: {comp}')
...
def main_umap(train_folders: list):
    ...
    if config.PLOT_GRAPHS:
        ...
        if config.SAVE_GRAPHS_TO_FILE:
            fig_filename = f'hdb_soft_assignments_{time_str}'  # fig1.savefig(os.path.join(config.OUTPUT_PATH, str.join('', ('hdb_soft_assignments', time_str, '.svg'))))
