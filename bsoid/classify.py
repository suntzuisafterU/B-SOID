...
# # # B-SOiD imports # # #
from bsoid.util import videoprocessing, visuals
from bsoid.config.LOCAL_CONFIG import BASE_PATH, COMP, FPS, FRAME_DIR, ID, GEN_VIDEOS, MODEL_NAME, OUTPUT_PATH, PLOT_TRAINING
from bsoid.config.GLOBAL_CONFIG import SVM_PARAMS
# from bsoid import BODYPARTS, COMP, FRAME_DIR, FPS, FRAME_DIR, ID, GEN_VIDEOS, MODEL_NAME, OUTPUT_PATH, PLOT_TRAINING, PREDICT_FOLDERS, TRAIN_FOLDERS, VID_NAME
...
def bsoid_frameshift_py(data_new, scaler, fps, model):
    ...
    logging.info(f'Done frameshift-predicting a total of {len(data_new)} files.')
    return labels_fshigh
def bsoid_frameshift_umap(data_new, fps, clf):
    """
    Frame-shift paradigm to output behavior/frame
    :param data_new: list, new data from predict_folders
    :param fps: scalar, argument specifying camera frame-rate in LOCAL_CONFIG
