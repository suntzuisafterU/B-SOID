...
from bsoid import classify, config, train
from bsoid.config import video_fps, MODEL_NAME, OUTPUT_PATH, TRAIN_FOLDERS, PREDICT_FOLDERS
...
def build_py(train_folders) -> Tuple[Any, Any, Any, Any, Any, Any]:
    ...
    multi_index_columns = pd.MultiIndex.from_tuples([('Features', 'Relative snout to forepaws placement'),
    ...
    with open(os.path.join(OUTPUT_PATH, f'bsoid_{MODEL_NAME}.csv'), 'wb') as f:
...
def run_py(predict_folders):
    ...
    for i, fd in enumerate(predict_folders):  # Loop through folders
        f = likelihoodprocessing.get_filenames(fd)
    ...
    for i in range(0, len(feats_new)):
        ...
        predictions.to_csv((os.path.join(OUTPUT_PATH, str.join('', ('bsoid_labels_10Hz', timestr, csvname,  '.csv')))),
                           index=True, chunksize=10000, encoding='utf-8')
        runlen_df1, dur_stats1, df_tm1 = statistics.main(labels_fslow[i])
        # if PLOT_TRAINING:
        ...
        xyfs_df.to_csv((os.path.join(OUTPUT_PATH, str.join('', ('bsoid_labels_', str(video_fps), 'Hz', timestr, csvname,
                                                                '.csv')))),
                       index=True, chunksize=10000, encoding='utf-8')
        runlen_df2, dur_stats2, df_tm2 = statistics.main(labels_fshigh[i])
...
def run_umap(predict_folders):
    ...
    data_new, fs_labels = bsoid_umap.classify.main(predict_folders, video_fps, nn_classifier)
    filenames = []
    all_df = []
    for i, fd in enumerate(predict_folders):  # Loop through folders
        f = get_filenames(fd)
    ...
    for i in range(0, len(fs_labels)):
        ...
        runlen_df.to_csv((os.path.join(OUTPUT_PATH, str.join('', ('bsoid_runlen_', str(video_fps), 'Hz', timestr, csvname,
                                                                  '.csv')))),
                         index=True, chunksize=10000, encoding='utf-8')
        dur_stats.to_csv((os.path.join(OUTPUT_PATH, str.join('', ('bsoid_stats_', str(video_fps), 'Hz', timestr, csvname,
                                                                  '.csv')))),
                         index=True, chunksize=10000, encoding='utf-8')
        df_tm.to_csv((os.path.join(OUTPUT_PATH, str.join('', ('bsoid_transitions_', str(video_fps), 'Hz', timestr, csvname,
                                                              '.csv')))),
                     index=True, chunksize=10000, encoding='utf-8')
        if config.PLOT_TRAINING:
            fig = plot_tmat(df_tm)
            my_file = 'transition_matrix'
            fig.savefig(os.path.join(OUTPUT_PATH, str.join('', (my_file, str(video_fps), 'Hz', timestr, csvname, '.svg'))))
...
def run_voc(predict_folders):
    ...
    data_new, feats_new, labels_fslow, labels_fshigh = bsoid_voc.classify.main(predict_folders, video_fps, behv_model)
    filenames = []
    all_df = []
    for i, fd in enumerate(predict_folders):  # Loop through folders
        f = get_filenames(fd)
